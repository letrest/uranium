  ...
  interactions.carousel = function ( fragment ) {
    ...
    function Carousel(set) {
      ...
      self.options = {
        ...
        cloneLength: [0,0],     // number of clones at front and back of carousel
      ...
      function initialize() {
        ...
        insertClones();
        updateIndex(self.options.center ? self.itemIndex + self.options.cloneLength[0] : self.itemIndex);
        updateDots();
        self.update();
      ...
      function readAttributes() {
        ...
        if (cloneLength) {
          cloneLength = $.map(cloneLength.split(","), function(i) { return parseInt(i); });
          self.options.cloneLength = cloneLength.length == 1 ? [0, cloneLength[0]] : cloneLength;
        }
        $container.attr("data-ur-clones", self.options.center ? self.options.cloneLength : self.options.cloneLength[1]);
      ...
      function insertClones() {
        if (!self.options.infinite) {
          self.options.cloneLength = [0, 0];
          $container.attr("data-ur-clones", 0);
          return;
        }

        if (self.options.cloneLength.join() == "0,0") {
          if (self.options.fill) {
            if (self.options.center)
              self.options.cloneLength = [self.options.fill - 1, self.options.fill - 1];
            else
              self.options.cloneLength = [0, self.options.fill];
          }
        ...
        for (var i = 0; i < self.options.cloneLength[1]; i++) {
        ...
        if (self.options.center) {
          ...
          var offset =  self.count - (self.options.cloneLength[0] % self.count);
          for (var i = offset; i < offset + self.options.cloneLength[0]; i++) {
            var srcIndex = i % self.count;
            var clone = $items.eq(srcIndex).clone(true).attr("data-ur-clone", srcIndex).attr("data-ur-state", "inactive");
            frag.appendChild(clone[0]);
          }
      ...
      function updateIndex(newIndex) {
        ...
        if (self.options.infinite && self.options.center)
          realIndex = self.itemIndex - self.options.cloneLength[0];
      ...
      function continueSwipe(e) {
        ...
        if (coords !== null) {
          ...
          if (self.options.infinite) {
            if (self.options.center) {
              var beginLimit = Math.floor(offsetFront($items[self.options.cloneLength[0]]) - viewport/2);
              var endLimit = Math.ceil(offsetFront($items[self.count + self.options.cloneLength[0]]) - viewport/2);
              var offset = offsetFront($items[self.count + self.options.cloneLength[0]]) - offsetFront($items[self.options.cloneLength[0]]); // length of all original items
      ...
      function moveTo(direction) {
        ...
        if (self.options.infinite) {
          ...
          if (self.options.center) {
            var offset = offsetFront($items[self.count + self.options.cloneLength[0]]) - offsetFront($items[self.options.cloneLength[0]]);
            if (newIndex < self.options.cloneLength[0]) { // clone at start of carousel
              translateX(transform - offset);
              newIndex += self.count;
              self.itemIndex = newIndex + direction;
            }
            else if (newIndex >= self.count + self.options.cloneLength[0]) { // clone at end of carousel
              translateX(transform + offset);
              newIndex -= self.count;
              self.itemIndex = newIndex + direction;
            }
      ...
      var zeroWidth = false;
      if (self.options.infinite && !self.options.fill && self.options.cloneLength[1] == 0) {
